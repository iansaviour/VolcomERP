Public Class FormSalesPromoSingle 
    ...
    Private Sub GVProduct_FocusedRowChanged(ByVal sender As System.Object, ByVal e As DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs) Handles GVProduct.FocusedRowChanged
        ...
        Dim id_design_costx As String = "0"
        ...
        Try
            ...
            id_design_costx = GVProduct.GetFocusedRowCellValue("id_design_cost").ToString
        Catch ex As Exception
        End Try

        'images
        pre_viewImages("2", PictureEdit1, id_designx, False)
        viewRetailPrice(id_design_costx, id_designx)
    ...
    Private Sub BtnChoose_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnChoose.Click
        ...
        If qty_input > 0 Then
            If action_pop = "ins" Then
                ...
                For i As Integer = 0 To (FormSalesPromoDet.GVItemList.RowCount - 1)
                    Dim id_product_cek As String = 0
                    Dim id_price_cek As String = 0
                    Dim design_price As Decimal = 0.0
                    Try
                        id_product_cek = FormSalesPromoDet.GVItemList.GetRowCellValue(i, "id_product").ToString
                        id_price_cek = FormSalesPromoDet.GVItemList.GetRowCellValue(i, "id_design_price").ToString
                        design_price = Decimal.Parse(FormSalesPromoDet.GVItemList.GetRowCellValue(i, "design_price").ToString)
                    Catch ex As Exception
                    End Try

                    'MsgBox(id_product_cek + " " + GVProduct.GetFocusedRowCellValue("id_product").ToString)
                    'MsgBox(id_price_cek + " " + GVProduct.GetFocusedRowCellValue("id_design_price").ToString)
                    'MsgBox(design_price + " ")
                ...
                Else
                    ...
                    'FormSalesPromoDet.sayCurr()
                    FormSalesPromoDet.getNetto()
                    FormSalesPromoDet.getVat()
                    FormSalesPromoDet.getTaxBase()
                    FormSalesPromoDet.GVItemList.OptionsBehavior.AutoExpandAllGroups = True
                    'FormSalesPromoDet.GVItemList.FocusedRowHandle = find_row(FormSalesReturnOrderDet.GVItemList, "id_product", GVProduct.GetFocusedRowCellValue("id_product").ToString)
            ...
            ElseIf action_pop = "upd" Then
                ...
                ElseIf is_higher Then
                    errorCustom("Qty cannot exceed  " + qty_limit.ToString + " !")
                Else
                    FormSalesPromoDet.GVItemList.SetFocusedRowCellValue("name", GVProduct.GetFocusedRowCellValue("name").ToString)
                    FormSalesPromoDet.GVItemList.SetFocusedRowCellValue("product_name", GVProduct.GetFocusedRowCellValue("product_name").ToString)
                    FormSalesPromoDet.GVItemList.SetFocusedRowCellValue("code", GVProduct.GetFocusedRowCellValue("code").ToString)
                    FormSalesPromoDet.GVItemList.SetFocusedRowCellValue("size", GVProduct.GetFocusedRowCellValue("size").ToString)
                    FormSalesPromoDet.GVItemList.SetFocusedRowCellValue("color", GVProduct.GetFocusedRowCellValue("color").ToString)
                    FormSalesPromoDet.GVItemList.SetFocusedRowCellValue("sales_pos_det_qty", SPQty.EditValue)
                    FormSalesPromoDet.GVItemList.SetFocusedRowCellValue("id_design_price", GVProduct.GetFocusedRowCellValue("id_design_price").ToString)
                    FormSalesPromoDet.GVItemList.SetFocusedRowCellValue("design_price_type", GVProduct.GetFocusedRowCellValue("design_price_type").ToString)
                    FormSalesPromoDet.GVItemList.SetFocusedRowCellValue("design_price", Decimal.Parse(GVProduct.GetFocusedRowCellValue("design_price").ToString))
                    FormSalesPromoDet.GVItemList.SetFocusedRowCellValue("id_design_price_retail", SLEPrice.EditValue.ToString)
                    FormSalesPromoDet.GVItemList.SetFocusedRowCellValue("design_price_retail", Decimal.Parse(SLEPrice.Properties.GetDisplayValueByKeyValue(SLEPrice.EditValue).ToString))
                    FormSalesPromoDet.GVItemList.SetFocusedRowCellValue("sales_pos_det_amount", Decimal.Parse(TxtAmount.EditValue.ToString))
                    FormSalesPromoDet.GVItemList.SetFocusedRowCellValue("id_design", GVProduct.GetFocusedRowCellValue("id_design").ToString)
                    FormSalesPromoDet.GVItemList.SetFocusedRowCellValue("id_product", GVProduct.GetFocusedRowCellValue("id_product").ToString)
                    FormSalesPromoDet.GVItemList.SetFocusedRowCellValue("id_sample", GVProduct.GetFocusedRowCellValue("id_sample").ToString)
                    FormSalesPromoDet.GCItemList.RefreshDataSource()
                    FormSalesPromoDet.GVItemList.RefreshData()
                    FormSalesPromoDet.getNetto()
                    FormSalesPromoDet.getVat()
