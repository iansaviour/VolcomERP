Public Class FormSampleStockOpname 
    ...
    Sub viewSOWH()
        Dim query As String = ""
        query += "SELECT lockx.lock, so.id_lock, so.id_sample_so, so.sample_so_number, so.id_report_status,stat.report_status, "
        query += "so.sample_so_date_created AS sample_so_date_created, "
        query += "so.sample_so_last_update AS sample_so_last_update, "
        ...
        query += "ORDER BY so.id_sample_so DESC "
