...
Public Class ReportMatRecWO
    ...
    Sub view_list_rec()
        ...
        query = "CALL view_wo_rec_mat_pcs('" & id_receive & "')"
    ...
    Private Sub ReportMatRecWO_BeforePrint(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintEventArgs) Handles MyBase.BeforePrint
        ...
        query += "INNER JOIN tb_season g ON g.id_season=h.id_season WHERE a.id_mat_wo_rec='" & id_receive & "'"
        Dim data As DataTable = execute_query(query, -1, True, "", "", "", "")
        ...
        LRecDate.Text = "DATE : " & view_date_from(data.Rows(0)("mat_wo_rec_date").ToString, 0)
        LDODate.Text = Date.Parse(data.Rows(0)("delivery_order_date").ToString).ToString("dd/MM/yyyy")

        LNote.Text = data.Rows(0)("mat_wo_rec_note").ToString

        view_list_rec()
        'load_mark("17", id_receive)
        load_mark_horz("17", id_receive, "2", "1", XrTable1)
    End Sub
    ...
    Private Sub PageFooter_BeforePrint(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintEventArgs) Handles PageFooter.BeforePrint
        LTotalReceived.Text = GVListPurchase.Columns("mat_wo_rec_det_qty").SummaryText.ToString
    End Sub
End Class